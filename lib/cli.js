...
void function () {
  var escodegen, escodegenFormat, fs, isAbsolutePath, js, nopt, options, outputPath, Path, pathOpt, resolveRelative, templates, xport;
  fs = require('fs');
  Path = require('path');
  nopt = require('nopt');
  xport = require('./');
  ...
  if (options.list) {
    templates = require('readr').sync(pathOpt, options).reduce(function (memo, file) {
      memo.push({
        path: file.path,
